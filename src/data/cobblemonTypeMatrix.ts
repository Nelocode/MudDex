import { PokemonType, TypeEffectivenessResult } from '../types/diosesmon';
...
// Single type attack multiplier chart: TYPE_CHART[attacker][defender]
...
export function calculateTypeEffectiveness(type1: PokemonType, type2?: PokemonType): TypeEffectivenessResult {
  ...
  ALL_TYPES.forEach(attacker => {
    let mult1 = TYPE_CHART[attacker][type1];
    let mult2 = type2 && type2 !== type1 ? TYPE_CHART[attacker][type2] : 1.0;
    let totalMult = mult1 * mult2;

    if (totalMult === 0) result.immunities0x.push(attacker);
    else if (totalMult >= 4) result.weaknesses4x.push(attacker);
    else if (totalMult > 1) result.weaknesses2x.push(attacker);
    else if (totalMult === 1) result.normal1x.push(attacker);
    else if (totalMult <= 0.25) result.resistances025x.push(attacker);
    else if (totalMult < 1) result.resistances05x.push(attacker);
  });
